...
- **Temporal Scaler**: Add opt-in `includeWorkerCapacity` (with `workerCapacityServerAddress`/`workerCapacityQuery`, and optional `workerCapacityHeaders`/`workerCapacityBearerToken`/`workerCapacityUnsafeSsl`) extending the same premature-scale-down protection as `includeRunningWorkflowCount` to fan-out workloads, where a single running Workflow drives many concurrent Activity executions against a fixed worker slot pool and a running-Workflow count of one gives no visibility into that load. Queries any Prometheus-compatible metrics source (e.g. scraping the Temporal SDK's own worker metrics) the user already runs; the scaler has no opinion on how those metrics are labeled or scraped. ([#XXX](https://github.com/kedacore/keda/issues/XXX))
- TODO ([#XXX](https://github.com/kedacore/keda/issues/XXX))
